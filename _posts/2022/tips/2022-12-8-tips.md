...
#### push local changes when remote branch has moved

This happens often when the remote branch has new commits and `git push` is rejected. A safe workflow is: fetch, rebase your local commit on top of the remote branch, resolve conflicts, then push.
...
# replay local commits on top of the latest remote branch
git rebase origin/master
```

If a conflict happens during rebase, resolve it file by file. For example, if you want to keep the remote version of `Gemfile.lock` and only keep your local content change in another file:

```bash
git checkout --ours Gemfile.lock
git add Gemfile.lock path/to/your_changed_file
git rebase --continue
```

Note: during `git rebase`, `--ours` means the branch you are rebasing onto, i.e. the current remote branch state. This is easy to confuse with normal merge behavior.
...
If `git fetch` fails because of a temporary network problem, just run it again. Do not use `git push --force` unless you are sure rewriting remote history is intended.
...
#### Resumable git clone (no starting over on disconnection)

Instead of `git clone`, use `git fetch` which can be retried without losing progress:

```bash
mkdir your_folder && cd your_folder
git init
git remote add origin https://github.com/your_username/your_repo.git
git fetch --depth=1 origin    # re-run this if it disconnects, until it succeeds
git checkout -b main FETCH_HEAD
```

If `git fetch` disconnects midway, just run it again — it resumes from what was already downloaded.

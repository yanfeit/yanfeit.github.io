...
## 2.1 Internet Traffic Distribution
...
We will use Adam optimizer to find out the approximate optimal solution[^2]. In order to evaluate the solution, python-MIP package is used to get the true optimal value to evalute the gap. The optimality is defined as, 

$$
\text{Optimality} = 1 - \frac{Q - Q^*}{Q^*}. \tag{14}
$$

where $Q^*$ is the objective value of optimal solution from an Exact algorithm. $Q$ is the objective value of the nearly optimal solution from the approximate method. 

...
In the following case, we used an Adam optimizer to find out the approximate optimal solution[^2]. In order to evaluate the solution, python-MIP package which applies Coin-or branch and cut algorithm is used to get the true optimal value to evalute the gap. The optimality is defined as, 
...
For an online application, customers will be served one by one. The responding time is very limited to several microseconds. The sensitivity $p_{ij}$ is predicted by inference of the machine learning model. Equation.(9) can be utilized in the inference engine to decide which coupon can be used for the incoming customer.


# 2. Simulated Case Study
## 2.1 Online Flow Control

In online advertisement, the web owner has $M$ advertisements to distribute. In order to optimize the total click rate, the problem can be formalize as follows,
...
where $s_j$ represents the ratio of the $j$th advertisements, $p_{ij}$ is the estimated click rate of the $j$th advertisements for the $i$th user, and $\epsilon$ is the minimal relaxation parameters to make sure an exact algorithm to have a valid solution. The Lagrangian of the Equation.(1) and its derivative are, 
...
To evaluate the feasibility of the primal-dual approximate method, we simulated several cases to compute the optimality and the constraint satisfaction. In the simulation, we set $\epsilon = 0.01$ and equal quiting condition in the interation of ADAM algorithm. In Fig.1, we can see that all cases can reach a nearly closed objective compared with results from Python-MIP solver.
...
For the constraint satisfaction, the primal-dual approximate method can hardly get all constraints satisfied. Therefore, it is inappropriate for this method to apply in any cases that the inequality constraints has to be meet with. However, for any soft constraints, this method can be helpful.

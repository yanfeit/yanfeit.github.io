...
那为什么说这是无监督学习呢？因为我们事先也不知道哪些数据点是归于哪个高斯分布的。而这，倒是我们期待从模型中得到的结果，也就是说我们希望知道到底我这个数据点归到各个类的概率是多少？我们把这个概率记作$$ p(k\|n) $$或者 $$p_{nk}$$，其中 $$ 0 \le k < K$$ 且 $$ 0 \le n < N$$。在文献中，$$ p_{nk}$$ 有时也被称为责任矩阵， 就是说第$$k$$个类需要对第$$n$$个数据点负多少责任。

总结下来，在给定一组数据，比如说 ($$N \times M$$) 的矩阵，我们希望估计以下的一组参数： 
$$
\boldsymbol{\mu}_k \\
\boldsymbol{\Sigma}_k \\
P(k|n) \equiv p_{nk} \tag{1}
$$
...
$$
p_{nk} \equiv P(k|n) = \frac{N(\mathbf{x}_n|\boldsymbol{\mu}_k, \boldsymbol{\Sigma}_k)P(k)}{P(\mathbf{x}_n)} \tag{5}
$$
...
\hat{\boldsymbol{\Sigma}}_k = \sum\limits_{n} p_{nk} (\mathbf{x}_n - \hat{\boldsymbol{\mu}}_k) \cdot (\mathbf{x}_n - \hat{\boldsymbol{\mu}}_k)^{\text{T}} / \sum\limits_{n} p_{nk} \\ \tag{6}
$$
类似地，
$$
